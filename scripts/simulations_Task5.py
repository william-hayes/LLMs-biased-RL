...
class HayesWedell2023CognitionTask_Standard:
    ...
    def __init__(self, model_name, trials_per_context=15, client=None, hf_token=None):
        ...
        freqO = np.array([[20, 25], [27, 26], [28, 27], [29, 34]])  # frequent outcomes (p = .80)
        rareO = np.array([[20, 10], [12, 26], [28, 42], [44, 34]])  # rare outcomes (p = .20)
        T = trials_per_context
        for i in range(4):
            for j in range(2):
                self.payoffs[i, j] = np.array([freqO[i, j]] * int(0.80 * T) + [rareO[i, j]] * int(0.20 * T))
    ...
    def append_feedback(self, options, outcomes, order):
        feedback = f"slot machine {options[0]} delivered {int(outcomes[order[0]])} dollars and slot machine {options[1]} delivered {int(outcomes[order[1]])} dollars.\n"
...
class HayesWedell2023CognitionTask_Comparisons(HayesWedell2023CognitionTask_Standard):
    def append_feedback(self, options, outcomes, order):
